...
- in the `App` component import `SendMail`
- on the `<nav>` element add two new `li`
  - Send Email control
  - Send Email un-control
- on the `<Switch>` element add `send-email` route that get `type` prams that only can be `control` or `uncontrol`.
